ent-CMGrenadeBase = граната
    .desc = Пригнуться и укрыться.
ent-RMCM40DPTags = { "" }
    .desc = { "" }
ent-RMCGrenadeFlashBang = светошумовая граната
    .desc = Отлично подходит для остановки групп бунтовщиков. Не ждите, что солнцезащитные очки защитят вас...
ent-CMGrenadeHighExplosive = граната M40 HEDP
    .desc = Взрывная граната двойного назначения. Небольшая, но обманчиво сильная фугасная граната, которая вытеснила гранаты M15 HE и заменила их на M40 HEFA. Может заряжаться в гранатомет M83 Launcher или метаться вручную.
ent-CMGrenadeSmoke = дымовая граната M40
    .desc = M40 HSDP - это небольшая, но мощная дымовая граната. Основана на той же платформе, что и M40 HEDP. Взрывается через 2 секунды.
ent-RMCSmoke = { ent-Smoke }
    .desc = { ent-Smoke.desc }
ent-CMGrenadeFrag = граната M40 HEFA
    .desc = Высокоэксплозивная осколочно-фугасная противопехотная граната. Небольшая, но обманчиво сильная осколочная граната, которая вытесняет осколочные гранаты M15, а также M40 HEDP. Может заряжаться в гранатомет M83 Launcher или метаться вручную.
ent-CMGrenadeFragOld = осколочная граната M15
    .desc = Устаревшая осколочная граната США. Прослужив несколько десятилетий в КМП США, старая осколочная граната M15 постепенно заменяется более безопасными гранатами серии M40. Она срабатывает через 4 секунды.
ent-RMCGrenadeTraining = учебная граната M07
    .desc = Безвредная многоразовая версия M40 HEDP, используемая для обучения. Может заряжаться в гранатомет или метаться вручную.
ent-RMCGrenadeIncendiary = зажигательная граната M40 HIDP
    .desc = M40 HIDP - это небольшая, но обманчиво сильная зажигательная граната, предназначенная для нарушения подвижности противника с помощью напалма типа B длительного действия. Она детонирует через 4 секунды.
ent-RMCGrenadeMolotov = коктель молотова
    .desc = Мощная импровизированная зажигательная бомба, соединенная с щепоткой пороха. Дешевая, очень эффективная и смертоносная в замкнутом пространстве. Часто встречается в руках повстанцев и террористов. Трудно предсказать, сколько секунд у вас будет до взрыва, поэтому будьте осторожны. Есть шанс, что она взорвется у вас перед носом.
ent-CMExplosionEffectGrenade = { "" }
    .desc = { "" }
ent-RMCExplosionEffectGrenadeShockWave = { "" }
    .desc = { "" }
ent-CMExplosionEffectShrapnel1 = { "" }
    .desc = { "" }
ent-CMExplosionEffectShrapnel2 = { ent-CMExplosionEffectShrapnel1 }
    .desc = { ent-CMExplosionEffectShrapnel1.desc }
